...
#else
    // UNLIT esetében KELLENEK ezek a sorok, mert a Graph nem generálja őket!
...
void MainLight_float(float3 WorldPos, out float3 Direction, out float3 Color, out float ShadowAtten)
{
    ...
    float rawAtten = mainLight.shadowAttenuation;
    
    // [DEBUG MODE] BRUTÁLIS ÁRNYÉK TESZT
    // Normál esetben: ShadowAtten = rawAtten;
    // Most: Ha az árnyék egy kicsit is sötétebb mint a tiszta fény (0.95), 
    // akkor legyen azonnal TELJESEN FEKETE (0.0).
    // Így tuti meglátod, ha működik.
    
    if (rawAtten < 0.95)
    {
        ShadowAtten = 0.0; // Koromfekete
    }
    else
    {
        ShadowAtten = 1.0; // Tiszta fény
    }
