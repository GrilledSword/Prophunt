...
{
    [Header("Sanity Audio")]
    [SerializeField] private AudioClip heartbeatClip; // Egy dobban�s hangja (vagy loop)
    [SerializeField] private float startSanityThreshold = 50f; // 50% alatt kezd�dik
    [SerializeField] private float maxVolume = 1.0f;
    [SerializeField] private float maxPitch = 1.5f; // Gyorsul�s m�rt�ke

    private HealthComponent healthComponent;
    private AudioSource audioSource;
    private PlayerNetworkController playerController;

    // Ha loop-os heartbeatet haszn�lsz
    ...
    private void Update()
    {
        // IsOwner ellen�rz�s a sz�l�n kereszt�l
        if (playerController == null || !playerController.IsOwner) return;

        // Csak Vad�sznak van Sanity effekt
